...
class PreparedStatement extends EventEmitter {
  ...
  input (name, type, value) {
    ...
    if (type === undefined) {
      for (const typemap in shared.map) {
        if (value instanceof typemap.js) {
          type = typemap.sql
          break
        }
      }

      throw new PreparedStatementError(`The type ${typeof value} is not registered to an mssql datatype.`)
    }
